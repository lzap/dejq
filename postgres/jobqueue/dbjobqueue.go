...
func New(url string) (*DBJobQueue, error) {
	...
	return NewFromGenericPool(NewPgxPool(pool))
}

func NewFromStdlib(db *sql.DB) (*DBJobQueue, error) {
	return NewFromGenericPool(NewStdlibPool(db))
}

func NewFromGenericPool(pool GenericPool) (*DBJobQueue, error) {
...
func (q *DBJobQueue) listen(ctx context.Context, ready chan<- struct{}) {
	...
	defer func() {
		// UNLISTEN must be called with background context because the main context has already been cancelled at this point
...
func (q *DBJobQueue) Dequeue(ctx context.Context, jobTypes []string, channels []string) (uuid.UUID, uuid.UUID, []uuid.UUID, string, json.RawMessage, error) {
	...
	if err := ctx.Err(); err != nil {
		return uuid.Nil, uuid.Nil, nil, "", nil, ErrDequeueTimeout
	}
...
func (q *DBJobQueue) DequeueByID(ctx context.Context, id uuid.UUID) (uuid.UUID, []uuid.UUID, string, json.RawMessage, error) {
	// Return early if the context is already canceled.
	if err := ctx.Err(); err != nil {
		return uuid.Nil, nil, "", nil, ErrDequeueTimeout
	}
	...
	if err == pgx.ErrNoRows {
		return uuid.Nil, nil, "", nil, ErrNotPending
	} else if err != nil {
...
func (q *DBJobQueue) jobDependencies(ctx context.Context, conn *GenericConn, id uuid.UUID) ([]uuid.UUID, error) {
